...
@router.post(
    "/signup/admin", response_model=UserOut, status_code=status.HTTP_201_CREATED
)
...
async def soft_delete_user_by_id(
    ...
    user_delete_status_check = await User.find_one(
        {"_id": user_id, "delete_status": True}
    )
    ...
    if user_check_in_trash:
        ...
        raise HTTPException(
            status_code=400, detail="User Already Permamently Deleted.."
        )
    dict_user = user.dict()
...
async def batch_soft_delete_users(
    ...
    if current_user.role == "user":
        raise HTTPException(
            status_code=403, detail="Only Admin Can Delete Multiple Records"
        )
    ...
    for user_id in valid_user_ids:
        ...
        user_check_in_trash = await Trash.find_one(
            {"original_data.id": ObjectId(user_id)}
        )
    ...
    if not deleted_users_data:
        raise HTTPException(status_code=404, detail="No users were deleted")
    return response
...
async def view_trash(
    ...
    if current_user.role == "user":
        raise HTTPException(
            status_code=403, detail="You Have Not Access To View Trash Data"
        )
    trash_records = await Trash.find().to_list()

    if not trash_records:
        raise HTTPException(
            status_code=404, detail="No soft-deleted user records found"
        )
    ...
    for record in trash_records:
        ...
        original_data = {
            k: convert_objectid_to_str(v) for k, v in record.original_data.items()
        }
...
async def restore_user_data(
    ...
    if current_user.role == "user":
        raise HTTPException(
            status_code=403, detail="Only Admin can Restore the Trash Record"
        )
    trash_record = await Trash.find_one({"original_data.id": ObjectId(user_id)})
    if not trash_record:
        raise HTTPException(status_code=404, detail="No soft-deleted user record found")
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error restoring user: {str(e)}")
...
async def permamently_delete_user_by_id(
    ...
    if not trash_record:
        raise HTTPException(
            status_code=404, detail="No User found for Permamently Deletion"
        )
    try:
        original_data = trash_record.original_data
        ...
        raise HTTPException(status_code=500, detail="Error during Deletion of Data")
    return {
...
async def upload_profile_picture(
    ...
    try:
        ...
        return {
            "message": f"Profile picture uploaded successfully.",
