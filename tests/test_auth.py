...
def test_login_user(client):
    ...
    response = client.post(
        "/auth/login", data={"username": "testing", "password": "testing"}
    )
...
def test_regenerate_refresh_token(client):
    ...
    login_res = client.post(
        "/auth/login", data={"username": "testadmin", "password": "testadmin"}
    )
    ...
    refresh_token_res = client.post(
        f"/auth/refresh/{refresh_token}", json={"refresh_token": refresh_token}
    )
